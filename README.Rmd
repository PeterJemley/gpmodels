...
tf = time_frame(
  ...
  step = hours(6),
  max_length = days(7),
  # optional parameter to limit to first 7 days of hospitalization
...
tf %>%
  add_rolling_predictors(
    variables = 'cr',
    # Note: You can supply a vector of variables
    lookback = hours(12),
    window = hours(6),
    stats = c(
      mean = mean,
  ...
  add_growing_predictors(variables = 'cr', # cumulative max creatinine since admission
                         stats = c(max = max)) %>%
  add_rolling_predictors(category = 'med',
                         # Note: category is always a regular expression
                         lookback = days(7),
                         stats = c(sum = sum)) %>%
